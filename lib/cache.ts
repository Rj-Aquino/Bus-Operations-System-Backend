import { redis } from './redis';

// Export all cache keys here for reuse
export const CACHE_KEYS = {
  STOPS_LIST: process.env.STOPS_CACHE_KEY,
  TICKET_TYPES : process.env.TICKET_TYPES_LIST,
  DASHBOARD : process.env.DASHBOARD_CACHE_KEY,
  
  ROUTES : process.env.ROUTES_CACHE_KEY,
  ROUTES_FULL : process.env.ROUTES_FULL_CACHE_KEY, 

  BUS_ASSIGNMENTS : process.env.BUS_ASSIGNMENTS_CACHE_KEY,

  BUS_OPERATIONS_ALL: process.env.BUS_OPERATIONS_CACHE_KEY_ALL,
  BUS_OPERATIONS_NOTREADY: process.env.BUS_OPERATIONS_CACHE_KEY_NOTREADY,
  BUS_OPERATIONS_NOTSTARTED: process.env.BUS_OPERATIONS_CACHE_KEY_NOTSTARTED,
  BUS_OPERATIONS_INOPERATION: process.env.BUS_OPERATIONS_CACHE_KEY_INOPERATION,

  RENTAL_REQUESTS_ALL: process.env.RENTAL_REQUESTS_CACHE_KEY_ALL,
  RENTAL_REQUESTS_PENDING: process.env.RENTAL_REQUESTS_CACHE_KEY_PENDING,
  RENTAL_REQUESTS_APPROVED: process.env.RENTAL_REQUESTS_CACHE_KEY_APPROVED,
  RENTAL_REQUESTS_REJECTED: process.env.RENTAL_REQUESTS_CACHE_KEY_REJECTED,
  RENTAL_REQUESTS_COMPLETED: process.env.RENTAL_REQUESTS_CACHE_KEY_COMPLETED,

  RENTAL_OPERATIONS_ALL: process.env.RENTAL_OPERATIONS_CACHE_KEY_ALL,
  RENTAL_OPERATIONS_NOTREADY: process.env.RENTAL_OPERATIONS_CACHE_KEY_NOTREADY,
  RENTAL_OPERATIONS_NOTSTARTED: process.env.RENTAL_OPERATIONS_CACHE_KEY_NOTSTARTED,
  RENTAL_OPERATIONS_INOPERATION: process.env.RENTAL_OPERATIONS_CACHE_KEY_INOPERATION,

};

export async function getCache<T = any>(key: string): Promise<T | null> {
  const cached = await redis.get(key);
  if (!cached) return null;
  try {
    return (typeof cached === 'string' ? JSON.parse(cached) : cached) as T;
  } catch {
    await redis.del(key);
    return null;
  }
}

export async function setCache(key: string, value: any, ttlSeconds: number = 3600) {
  await redis.set(key, JSON.stringify(value), { ex: ttlSeconds });
}

export async function delCache(key: string) {
  await redis.del(key);
}

export async function clearAllCache() {
  await redis.flushdb();
}

